...
export function mvcProvide<TProps>(Component: React.ComponentClass<PropsWithMvcProvider<TProps>> | React.FunctionComponent<PropsWithMvcProvider<TProps>>) {
  const provideComponent: React.FunctionComponent<TProps> = function (props) {
    return <mvcControllerContext.Consumer>{(mvcController) => <Component mvcController={mvcController} {...props} />}</mvcControllerContext.Consumer>;
  };
